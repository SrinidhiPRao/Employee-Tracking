  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        {lateThreshold && (
          <p className="text-xs text-gray-400 mb-4">Threshold: {lateThreshold}</p>
        )}
              ...
              <tbody>
                {lateRecords.map((record, index) => (
                  <tr key={index} className="border-b border-gray-50 last:border-0">
                    ...
                    <td className="py-2">{new Date(record.check_in).toLocaleTimeString()}</td>
                    <td className="py-2 text-red-500">{record.late_by}</td>
                  </tr>
                ))}
              </tbody>
            </table>
